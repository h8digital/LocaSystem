...
interface BtnProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?:string; size?:string; loading?:boolean; icon?:string; children?:React.ReactNode
}
export default function Btn({ variant='primary', size='md', loading, icon, children, className='', disabled, ...props }: BtnProps) {
  return (
    <button {...props} disabled={disabled||loading}
      ...
      {loading&&<span className="ds-spinner" style={{width:13,height:13,display:'inline-block'}}/>}
